...
function Add-DnsTxt {
    ...
    $apiRoot = "https://api.godaddy.com/v2/customers/$BSCustomerId/domains"
    if ($BSUseOTE) {
        $apiRoot = "https://api.ote-godaddy.com/v2/customers/$BSCustomerId/domains"
    }

    $headers = @{Authorization = "sso-key $($BSKey):$([pscredential]::new('a',$BSSecret).GetNetworkCredential().Password)"}

    if (-not ($zone = Find-BSZone $RecordName $headers $apiRoot)) {
    ...
    if (-not $recs -or $TxtValue -notin $recs.data) {
        ...
        $recs = @(
            $recs |
            Where-Object { -not [string]::IsNullOrWhiteSpace($_.data) } |
            Select-Object -Property data, ttl
        )

        if (!$recs -or $recs.Count -eq 0) {
            # Build the new record set from scratch
            $bodyJson = "[{`"data`":`"$TxtValue`",`"ttl`":3600}]"
        } else {
            # add the new record and build the body
            $recsNew = $recs + ([pscustomobject]@{data=$TxtValue;ttl=3600})
    ...
    <#
    .SYNOPSIS
        Add a DNS TXT record to Brandsight.
...
function Remove-DnsTxt {
    ...
    if (-not $recs -or $TxtValue -notin $recs.data) {
        ...
        # For some odd reason, the Brandsight API doesn't have a method to delete a
        ...
        if ($recs.Count -le 1) {
            ...
            $recsNew = @(
                $recs |
                Where-Object { $_.data -ne $TxtValue } |
                Select-Object -Property data, ttl
            )
            $bodyJson = ConvertTo-Json @($recsNew) -Compress
            $method = "Put"
    ...
    <#
    .SYNOPSIS
        Remove a DNS TXT record from Brandsight.
...
function Find-BSZone {
    ...
    if (!$script:BSRecordZones) { $script:BSRecordZones = @{} }

    # check for the record in the cache
    if ($script:BSRecordZones.ContainsKey($RecordName)) {
        return $script:BSRecordZones.$RecordName
    }
    ...
    for ($i=0; $i -lt ($pieces.Count-1); $i++) {
        ...
        # Even though Brandsight doesn't officially support sub-zone DNS hosting, it's possible
        # to add a "domain" that is technically a sub-zone of an actual domain registered
        # elsewhere and just delegate to Brandsight's nameservers. The web UI and API won't list
        ...
        $script:BSRecordZones.$RecordName = $zoneTest
        return $zoneTest
    }
...
# API Docs:
# https://developer.Brandsight.com/doc/endpoint/domains
